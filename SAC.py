...
class Critic(nn.Module):
    def __init__(self, action_size, state_size, hidden_size=64):
        ...
        self.C1 = nn.Linear(state_size, hidden_size)
        self.C2 = nn.Linear(hidden_size, hidden_size)
        self.C3 = nn.Linear(hidden_size, action_size)
...
class SAC(DQN):
    ...
    def q_target(self, next_state, reward, dones):
        with torch.no_grad():
            ...
            q1 = self.critic1_target(next_state)
            q2 = self.critic2_target(next_state)

            min_q = torch.sum(next_probs * torch.min(q1, q2), dim=1, keepdim=True)  # 其实就是价值函数

            # 原式中求的是Q、熵的均值。
            td_target = reward + self.gamma * (min_q + self.log_alpha.exp() * entropy) * (1 - dones)
        return td_target
