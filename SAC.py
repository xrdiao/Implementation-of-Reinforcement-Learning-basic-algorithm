...
class SAC(DQN):
    ...
    def q_target(self, next_state, reward, dones):
        ...
        entropy = Categorical(next_probs).entropy().view(-1, 1)

        q1_value = self.critic1_target(next_state)
        q2_value = self.critic2_target(next_state)

        mean_q = torch.sum(next_probs * torch.min(q1_value, q2_value), dim=1, keepdim=True)  # 其实就是价值函数

        # 原式中求的是Q、熵的均值。
        td_target = reward + self.gamma * (mean_q + self.log_alpha.exp() * entropy) * (1 - dones)
        return td_target
    ...
    def learn(self, state_, action_, reward_, next_state_, dones_):
        ...
        q1 = self.critic1(state_).gather(1, action_)
        loss_critic1 = torch.mean(F.mse_loss(q1, td_target.detach()))
        q2 = self.critic2(state_).gather(1, action_)
        ...
        q1 = self.critic1(state_)
        q2 = self.critic2(state_)
        mean_q = torch.sum(probs * torch.min(q1, q2), dim=1, keepdim=True)

        self.optim_actor.zero_grad()
        loss_actor = torch.mean(-self.log_alpha.exp() * entropy - mean_q)
        loss_actor.backward()
        self.optim_actor.step()

        # 自动调节alpha但结果容易发散
        # self.optim_alpha.zero_grad()
        # loss_alpha = torch.mean(self.log_alpha.exp() * (entropy - self.target_entropy).detach())
        # loss_alpha.backward()
