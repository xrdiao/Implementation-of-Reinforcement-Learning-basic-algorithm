...
**离散动作：**训练流程和PG一样，和PPO-Penalty相比少了个KL散度的计算，整体编写更加简单，但是效果更好。

**连续动作：**将离散动作中使用的分类分布 `torch.distributions.Categorical()` 更改为正态分布 `torch.distributions.Normal()`，其余不变。但在实现的过程中，出现了神经网络输出为 `nan` 的情况
